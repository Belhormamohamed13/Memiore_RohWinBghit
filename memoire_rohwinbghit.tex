...
%─── Tables et listes (numérotation romaine) ───────────────────────────────────
\pagenumbering{roman}
\setcounter{page}{1}
...
\begin{itemize}
  ...
  \item \textbf{Paiement local extensible.} Trois méthodes de paiement coexistent~: le règlement en espèces opérationnel, et l'intégration des cartes de paiement \textsc{cib} et \textit{Edahabia} via la passerelle de paiement en ligne \textbf{Chargily Pay V2} (en mode \textit{sandbox} pour les tests). L'architecture, fondée sur le patron \textit{Strategy/Factory}, est conçue pour être extensible à d'autres passerelles comme \textit{Stripe} et \textit{PayPal} pour les usages internationaux.
...
\begin{figure}[H]
\centering
\includegraphics[height=0.6\textheight,keepaspectratio]{fig_use_case.png}
...
Le modèle est organisé autour de la classe \texttt{User}, qui agrège les informations communes à tous les utilisateurs. Le rôle de l'utilisateur (passager, conducteur, administrateur) est directement géré via un attribut \texttt{role} de type énuméré au sein de la classe \texttt{User}, simplifiant l'architecture sans classe d'association intermédiaire. Le domaine s'articule autour de quatre agrégats~:
\begin{itemize}
  \item \textbf{Transport~:} \texttt{Trip}, \texttt{TripRequest}, \texttt{routes}, \texttt{Booking}, \texttt{Vehicle}.
  \item \textbf{Paiement~:} \texttt{PaymentSession}, \texttt{payment\_methods}, \texttt{Wallet}, \texttt{WalletTransaction}, \texttt{SurgePricingRule}.
...
\hline
\texttt{User} & id, phone, email, password, first\_name, last\_name, date\_of\_birth, role, identity\_status, created\_at & register(), verifyOtp(), completeKyc(), updateProfile() \\
\hline
\texttt{Vehicle} & id, owner\_id, make, model, license\_plate\_encrypted, color, seats\_available, is\_verified & register(), update(), deactivate() \\
\hline
...
\hline
\texttt{TripRequest} & id, passenger\_id, from\_wilaya\_code, to\_wilaya\_code, date, seats, status, expires\_at & create(), match(), expire() \\
\hline
...
\hline
\texttt{Booking} & id, trip\_id, passenger\_id, num\_seats, total\_price, status, payment\_status, payment\_method, created\_at & confirm(), cancel(), generateQr() \\
\hline
...
\hline
\texttt{IdentityVerification} & id, user\_id, id\_card\_front\_url, id\_card\_back\_url, video\_url, face\_match\_score, liveness\_score, antispoof\_score, status & submit(), processAI(), reviewManually() \\
\hline
\texttt{FraudEvent} & id, user\_id, verification\_id, event\_type, severity, reason, created\_at & flag(), investigate() \\
\hline
\texttt{FaceEmbedding} & id, user\_id, embedding, version, created\_at & generate(), compare() \\
\hline
\texttt{Wallet} & id, user\_id, balance, currency & credit(), debit() \\
\hline
\texttt{WalletTransaction} & id, wallet\_id, type, amount, balance\_after, reference\_id, reference\_type, created\_at & record(), reverse() \\
\hline
\texttt{Review} & id, booking\_id, trip\_id, reviewer\_id, reviewee\_id, rating, comment, created\_at & submit(), moderate() \\
\hline
...
\hline
\texttt{Notification} & id, user\_id, type, title, message, is\_read, created\_at & dispatch(), markAsRead() \\
\hline
\texttt{SurgePricingRule} & id, wilaya\_code, day\_of\_week, hour\_start, hour\_end, multiplier, is\_active & evaluate(), cache() \\
...
\hline
verification\_status & VARCHAR(20) & ENUM, défaut \textit{unverified} \\
\hline
...
\hline
driver\_id & UUID & FK \texttt{users.id}, NOT NULL \\
\hline
vehicle\_id & UUID & FK \texttt{vehicles.id}, NOT NULL \\
\hline
from\_wilaya\_id & INTEGER & FK \texttt{wilayas.id}, NOT NULL \\
\hline
to\_wilaya\_id & INTEGER & FK \texttt{wilayas.id}, NOT NULL \\
\hline
departure\_time & TIMESTAMPTZ & NOT NULL, $>$ NOW() \\
\hline
price\_per\_seat & DECIMAL(10,2) & NOT NULL, $>$ 0 \\
\hline
available\_seats & INTEGER & NOT NULL, dans [0, capacité] \\
\hline
...
id & UUID & Clé primaire \\
\hline
trip\_id & UUID & FK \texttt{trips.id}, NOT NULL \\
\hline
passenger\_id & UUID & FK \texttt{users.id}, NOT NULL \\
\hline
num\_seats & INTEGER & NOT NULL, $>$ 0 \\
\hline
total\_price & DECIMAL(10,2) & num\_seats $\times$ price\_per\_seat, NOT NULL \\
\hline
status & VARCHAR(20) & ENUM (\textit{pending / confirmed / cancelled / completed}) \\
\hline
payment\_status & VARCHAR(20) & ENUM (\textit{pending / paid / refunded / failed}) \\
\hline
\end{tabularx}
...
id & UUID & Clé primaire \\
\hline
user\_id & UUID & FK \texttt{users.id}, UNIQUE par rôle \\
\hline
id\_card\_front\_url & VARCHAR(255) & Cloudinary, chiffré au repos \\
\hline
id\_card\_back\_url & VARCHAR(255) & Cloudinary \\
\hline
face\_match\_score & DECIMAL(5,4) & Similarité InsightFace [0,1] \\
\hline
liveness\_score & DECIMAL(5,4) & Détection de vivacité [0,1] \\
\hline
...
\texttt{routes}(\underline{id}, \#from\_wilaya\_id, \#to\_wilaya\_id, from\_city, to\_city, distance\_km, duration\_minutes, route\_geometry, waypoints)\\[1pt]
\texttt{trips}(\underline{id}, \#driver\_id, \#vehicle\_id, \#from\_wilaya\_id, \#to\_wilaya\_id, departure\_time, price\_per\_seat, available\_seats, status, created\_at)\\[1pt]
...
\texttt{identity\_verifications}(\underline{id}, \#user\_id, id\_card\_front\_url, id\_card\_back\_url, selfie\_url, video\_url, face\_match\_score, liveness\_score, status, created\_at)\\[1pt]
...
\paragraph{Services tiers, communication et supervision.} Afin d'assurer la résilience opérationnelle, la distribution asynchrone des tâches et la supervision en temps réel de la plateforme, plusieurs services et bibliothèques industriels ont été intégrés~:
\begin{itemize}
  ...
  \item \textbf{Sentry SDK (Monitoring et observabilité) :} Intégré au niveau du client mobile, du client web et du serveur API pour la capture proactive des rapports d'erreurs en production et le profilage des temps de réponse.
...
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/17_kyc_intro.png}
...
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/21_kyc_verification_en_cours.png}
...
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/04_accueil_passager.png}
...
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/05_recherche_resultats.png}
...
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/06_recherche_details_reservation.png}
    {\footnotesize \textbf{3.4a} --- Détail du trajet\\Profil conducteur, itinéraire.\par}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/07_ticket_attente_qr.png}
    {\footnotesize \textbf{3.4b} --- Billet en attente\\Statut en attente, code QR.\par}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/07_ticket_confirme_qr.png}
    {\footnotesize \textbf{3.4c} --- Billet confirmé\\Statut confirmé, code QR.\par}
\end{minipage}\hfill
\begin{minipage}{0.24\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/live_tracking.png}
    {\footnotesize \textbf{3.4d} --- Suivi GPS temps réel\\Position Mapbox, bouton SOS.\par}
\end{minipage}
\caption{Parcours passager --- détail trajet, cycle du billet et suivi GPS}
...
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/tab_trips.png}
...
\begin{minipage}{0.31\textwidth}
    ...
    {\footnotesize \textbf{3.7b} --- Redirection de paiement\\Formulaire \textsc{cib}/\textit{Edahabia}, passerelle \textbf{Chargily Pay V2}.\par}
\end{minipage}\hfill
\begin{minipage}{0.31\textwidth}
    \centering
...
\label{fig:seq-checkin-mutuel-mono}
\end{figure}
...
\label{fig:act-checkin-mutuel-mono}
\end{figure}
...
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/09_accueil_conducteur.png}
    {\footnotesize \textbf{3.8a} --- Écran d'accueil conducteur\\Première impression, slogan, CTA.\par}
\end{minipage}\hfill
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/10_publier_trajet_formulaire.png}
...
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/14_portefeuille_solde.png}
    {\footnotesize \textbf{3.10a} --- Portefeuille conducteur\\Solde, historique, retrait bancaire.\par}
\end{minipage}\hfill
\begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/15_mes_vehicules_liste.png}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/KYC.png}
    \caption{Revue manuelle des dossiers \textsc{kyc} litigieux}
    \label{fig:admin-kyc}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/Gestione_Conducteur.png}
    \caption{Gestion des profils conducteurs et habilitations}
...
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/Gestione_reservation.png}
    \caption{Supervision et gestion en temps réel des réservations}
    \label{fig:admin-trips}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{Screen_Mobile/Verificatione_vehicules.png}
    \caption{Module de vérification des documents de véhicules}
...
Trois méthodes de paiement coexistent~: règlement en espèces (opérationnel), et cartes de paiement \textsc{cib} / \textit{Edahabia} (intégration en environnement \textit{sandbox} via la passerelle de paiement \textbf{Chargily Pay V2}). L'architecture, fondée sur le patron \textit{Strategy/Factory} décrit en section~\ref{sec:realisation-backend}, est extensible à \textit{Stripe} et \textit{PayPal} pour les usages internationaux.
...
\hline
A10 \textit{SSRF} &
Aucun appel HTTP sortant paramétrable par l'utilisateur~; URLs \textit{whitelistées} (Mapbox, \textsc{fcm}, Chargily Pay V2). &
--- \\
\hline
\end{tabular}
...
\begin{itemize}[leftmargin=1.8em]
  ...
  \begin{itemize}
    ...
    \item \textbf{Contrôleurs (\textit{controllers})~:} Affichent une couverture d'instructions de \textbf{71,85\,\%}, un excellent indicator confirmant que la logique métier de gestion des réservations, de matching de trajets, de messagerie asynchrone et de vérification biométrique est minutieusement vérifiée.
  \end{itemize}
\end{itemize}
...
\paragraph{Lecture des résultats.} La totalité des 111~tests unitaires et 262~tests d'intégration passent avec succès (100\,\% de réussite), ce qui atteste de la correction et de la robustesse de la logique métier centrale (authentification, tarification, scoring, détection de fraude, transitions d'état) ainsi que de l'interopérabilité fonctionnelle avec la base de données PostgreSQL, le cache Redis et le service d'IA.

\paragraph{Perspective.} Le maintien de cette couverture de tests à 100\,\% et l'automatisation dans le pipeline de CI/CD (GitHub Actions) permettront de pérenniser la stabilité du système lors des évolutions futures.
...
\paragraph{Intégration des paiements algériens.} L'intégration directe avec les serveurs de la \textsc{satim} pour le traitement des cartes \textsc{cib} et \textit{Edahabia} est un parcours d'homologation extrêmement complexe. \textit{Solution adoptée~:} la plateforme s'appuie sur la passerelle tierce de confiance \textbf{Chargily Pay V2} en mode \textit{sandbox} pour les cartes interbancaires, avec une architecture extensible (patron \textit{Factory}) facilitant le basculement vers la production. Le règlement en espèces, parfaitement opérationnel, assure la couverture immédiate.
...
\begin{itemize}
  ...
  \item \textbf{Qualité logicielle.} 111/111 tests unitaires en succès (100\,\%), 373/373 tests au global (100\,\% de réussite), mesures de performance en cohérence avec les objectifs.
...
\hline
Prototype fonctionnel & MVP complet~: 62 écrans, 373/373 tests, déployable & \textcolor{green!60!black}{\textbf{Conforme}} \\
\hline
...
\titleformat{name=\chapter,numberless}[block]
  {\normalfont\color{rohblue}\Large\bfseries}
  {}
  {0pt}
  {#1}
