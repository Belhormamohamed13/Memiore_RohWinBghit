...
\begin{highlight}
\small
\texttt{users}(\underline{id}, phone, email, password, first\_name, last\_name, date\_of\_birth, role, verification\_status, identity\_status, created\_at)\\[2pt]
\texttt{vehicles}(\underline{id}, \#owner\_id, make, model, license\_plate\_encrypted, color, seats, is\_active)\\[2pt]
\texttt{wilayas}(\underline{code}, name\_fr, name\_ar, latitude, longitude)\\[2pt]
\texttt{routes}(\underline{id}, \#from\_wilaya\_id, \#to\_wilaya\_id, from\_city, to\_city, distance\_km, duration\_minutes, route\_geometry, waypoints)\\[2pt]
\texttt{trips}(\underline{id}, \#driver\_id, \#vehicle\_id, \#from\_wilaya\_id, \#to\_wilaya\_id, departure\_time, price\_per\_seat, available\_seats, status, created\_at)\\[2pt]
\texttt{trip\_requests}(\underline{id}, \#passenger\_id, \#from\_wilaya\_code, \#to\_wilaya\_code, date, seats, status, expires\_at)\\[2pt]
\texttt{bookings}(\underline{id}, \#trip\_id, \#passenger\_id, num\_seats, total\_price, status, payment\_status, payment\_method, created\_at)\\[2pt]
\texttt{payment\_sessions}(\underline{id}, \#user\_id, \#booking\_id, provider, provider\_checkout\_id, payment\_method, amount, status, checkout\_url, created\_at)\\[2pt]
\texttt{payment\_methods}(\underline{id}, \#user\_id, type, card\_number\_encrypted, card\_number\_iv, card\_number\_auth\_tag, card\_number\_salt, last\_four, cardholder\_name, expiry\_month, expiry\_year, is\_default, created\_at)\\[2pt]
\texttt{identity\_verifications}(\underline{id}, \#user\_id, id\_card\_front\_url, id\_card\_back\_url, selfie\_url, video\_url, face\_match\_score, liveness\_score, status, created\_at)\\[2pt]
\texttt{fraud\_events}(\underline{id}, \#user\_id, \#verification\_id, event\_type, severity, reason, created\_at)\\[2pt]
\texttt{wallets}(\underline{id}, \#user\_id, balance, currency)\\[2pt]
\texttt{wallet\_transactions}(\underline{id}, \#wallet\_id, type, amount, balance\_after, reference\_id, reference\_type, created\_at)\\[2pt]
\texttt{chats}(\underline{id}, \#user1\_id, \#user2\_id, \#trip\_id, last\_message\_id, last\_message\_at)\\[2pt]
\texttt{messages}(\underline{id}, \#chat\_id, \#sender\_id, content, type, read\_at, created\_at)\\[2pt]
\texttt{notifications}(\underline{id}, \#user\_id, type, title, message, is\_read, created\_at)\\[2pt]
\texttt{reviews}(\underline{id}, \#booking\_id, \#trip\_id, \#reviewer\_id, \#reviewee\_id, review\_type, rating, comment, created\_at)\\[2pt]
\textit{Modules d'extensions :}\\[2pt]
\texttt{rides}(\underline{id}, \#passenger\_id, \#driver\_id, status, created\_at)\\[2pt]
\texttt{earnings}(\underline{id}, \#driver\_id, \#ride\_id, \#booking\_id, amount, earned\_at)\\[2pt]
\texttt{gift\_cards}(\underline{id}, code, amount, is\_redeemed, redeemed\_by, created\_at)\\[2pt]
\texttt{transport\_schedules}(\underline{id}, type, from\_city, to\_city, from\_station, to\_station, departure\_time, arrival\_time, price, company\_name, train\_class, seats\_available, is\_available, created\_at)\\[2pt]
