...
CREATE TABLE trips (
  ...
  from_wilaya_code  INT            NOT NULL REFERENCES wilayas(code),
  to_wilaya_code    INT            NOT NULL REFERENCES wilayas(code),
...
CREATE INDEX idx_trips_route_date ON trips
  (from_wilaya_code, to_wilaya_code, departure_date);
\end{lstlisting}
