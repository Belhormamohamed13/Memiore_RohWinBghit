\chapter{Exigences du système et Conception}
...
\section{Description des besoins et méthodologie}
\label{sec:methodologie}

La conception d'une plateforme distribuée requiert une démarche disciplinée. Nous avons retenu une approche itérative inspirée du Processus Unifié (\textsc{up}) allégé~\cite{kruchten_up_2004}, combinée à des itérations agiles de type \textit{Scrum} pour la réalisation.
...
\end{enumerate}
\subsection{Résultats de l'enquête utilisateur}
...
\subsection{Organisation du développement --- cadre Scrum}
...
Les récits utilisateurs associés au rôle de conducteur décrivent la publication de trajets inter-wilayas, la gestion des demandes de réservation et la configuration du profil du véhicule.

\subsection{Passager}
Les récits utilisateurs associés au rôle de passager décrivent la recherche de trajets avec filtres avancés, la réservation avec paiement multi-méthodes et la réception de notifications.

\subsection{Administrateur}
Les récits utilisateurs associés au rôle de l'administrateur décrivent la modération de la file d'attente KYC, la validation manuelle et la gestion générale des comptes utilisateurs.
...
\section{Modélisation de cas d'utilisation}
\label{sec:uc-global}
...
\textit{Scénario nominal~:} téléversement recto/verso de la \gls{cin}, capture vidéo de vivacité (3~s), analyse asynchrone par le service IA, décision automatique (verified/pending\_review/rejected).
...
\subsection{Diagramme d'activité global du parcours utilisateur}
...
\clearpage
\section{Diagrammes de séquences systèmes}
\label{sec:dss}

 Afin d'illustrer la dynamique du système, nous présentons dans cette section les diagrammes de séquences systèmes (DSS) pour les scénarios clés de la plateforme : l'inscription d'un utilisateur (Figure \ref{fig:seq-inscription}), la vérification d'identité KYC (Figure \ref{fig:seq-kyc}), le matching inversé (Figure \ref{fig:seq-trip-request}), et la réservation avec paiement multi-méthodes (Figure \ref{fig:seq-reservation}).
...
Le modèle intègre quatre patrons de conception~: \textit{Strategy} et \textit{Factory} pour les paiements, \textit{Repository} pour l'accès aux données, et \textit{Observer} pour la diffusion des événements. \subsection{Définition et concepts DCE}

Les diagrammes de classes participantes décrivent, pour chaque cas d'utilisation, les trois principales catégories de classes d'analyse et leurs relations :
\begin{itemize}
  \item \textbf{Les classes de dialogue :} représentent les interactions entre le système et les utilisateurs (les écrans Flutter pour l'application mobile et les composants React pour le dashboard web).
  \item \textbf{Les classes d'entités :} représentent les concepts métier et les données persistantes qui doivent être stockées en base de données PostgreSQL (ex. \texttt{User}, \texttt{Trip}, \texttt{Booking}).
  \item \textbf{Les classes de contrôles :} contiennent la logique de l'application, servant d'intermédiaires entre les dialogues et les entités (ex. contrôleurs REST et services Express.js).
\end{itemize}
