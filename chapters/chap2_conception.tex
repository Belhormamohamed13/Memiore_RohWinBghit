...
L'application mobile utilise React Native avec Expo.

\subsection{Architecture Backend}
Le backend est organisé en microservices Express.js/Node.js.

\subsection{Architecture Microservices}
Comprend le service de KYC (FastAPI) et de notification.

\subsection{Architecture Base de données}
PostgreSQL avec réplication et cache Redis.
...
Afin de mieux spécifier les besoins fonctionnels, nous modélisons les exigences sous forme d'User Stories regroupées par acteur.

\subsection{User Stories Conducteur}
\label{sec:us-conducteur}
Le conducteur inter-wilayas souhaite pouvoir publier des trajets, accepter ou refuser des réservations de passagers, configurer son profil de véhicule, et recevoir ses gains.

\subsection{User Stories Passager}
\label{sec:us-passager}
Le passager souhaite pouvoir rechercher un trajet inter-wilayas par filtres (date, wilaya), réserver une place en ligne, payer en toute sécurité (espèces ou e-paiement), et communiquer en temps réel avec le conducteur.

\subsection{User Stories Administrateur}
\label{sec:us-admin}
L'administrateur souhaite pouvoir valider les dossiers KYC des utilisateurs (carte d'identité, liveness), modérer les trajets signalés et superviser les transactions financières.



% Exigences fonctionnelles déplacées
...
\label{sec:uc-trajets}
Description détaillée de la gestion des trajets.

\subsection{Réservation}
\label{sec:uc-reservation}
Description détaillée de la réservation.

\subsection{Paiement}
\label{sec:uc-paiement}
Description détaillée du paiement.
...
Classes d'utilisateurs et rôles.

\subsection{Gestion des trajets}
Classes représentant les trajets.

\subsection{Gestion des réservations}
Classes de réservations.

\subsection{Gestion des paiements}
Classes pour le traitement des paiements.
...
Séquence d'authentification par code OTP.
