...
\section{Analyse des besoins et méthodologie de développement}
...
\clearpage
\section{Analyse fonctionnelle et modélisation des cas d'utilisation}
...
\begin{itemize}
  ...
  \item \textbf{Scénario nominal :}
    \begin{enumerate}
      ...
      \item Le microservice IA exécute l'analyse asynchrone (OCR, comparaison faciale, détection de vivacité).
...
Le flux \textsc{kyc} (figure~\ref{fig:seq-kyc}) constitue le scénario le plus critique~: téléversement du document, capture vidéo de vivacité, traitement asynchrone par le pipeline IA (comparaison faciale InsightFace, détection de vivacité, anti-\textit{spoofing}), décision automatique à trois états (\textit{verified}, \textit{pending\_review}, \textit{rejected}), et éventuelle revue manuelle. Les seuils de décision IA sont différenciés par rôle~: conducteur (score facial $\geq$~0{,}65, vivacité $\geq$~0{,}65, anti-\textit{spoofing} $\geq$~0{,}60)~; passager (0{,}60, 0{,}55, 0{,}50). Un \textit{hard-fail} (score $<$ 0,30) déclenche un rejet immédiat et la consignation d'un événement de fraude.
...
Le scénario de matching permet également au passager d'exprimer une demande de trajet, à la suite de laquelle le système identifie les conducteurs éligibles et notifie ces derniers (figure~\ref{fig:seq-trip-request}).
...
\begin{itemize}
  \item \textbf{UC-CHAT --- Messagerie interne en temps réel :}
    \begin{itemize}
      ...
      \item \textit{Scénario nominal :} Messages texte via Socket.IO, persistés en base de données.
    \end{itemize}
...
\clearpage
\section{Modélisation des processus métiers (BPMN)}
\label{sec:bpmn}
...
\clearpage
\section{Conception architecturale et déploiement}
...
\clearpage
\section{Modélisation statique et base de données}
\label{sec:conception-statique}

Cette section formalise la structure statique du domaine, le dictionnaire de ses classes et concepts métiers, le schéma relationnel qui en découle ainsi que les dictionnaires physiques de ses tables de données les plus critiques.

\subsection{Diagramme de classes du domaine}
\label{sec:classes}

La figure~\ref{fig:classes} représente le diagramme global de classes participantes du domaine (avec alignement \textsc{postgresql}).
...
\subsection{Modèle Conceptuel de Données (MCD) / Modèle relationnel (MLD)}
...
\label{sec:dictionnaires-donnees}
