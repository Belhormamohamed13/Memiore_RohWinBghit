...
\clearpage
\section{Spécification des scénarios dynamiques}
\label{sec:scenarios-dynamiques}
...
L'inscription repose sur une vérification du numéro de téléphone par code OTP, suivie de la saisie des informations personnelles (figure~\ref{fig:seq-inscription}). Le processus de connexion (authentification) ultérieur par code OTP pour un utilisateur enregistré est modélisé sous la forme d'un diagramme de séquence dans la figure~\ref{fig:seq-auth}.
...
\begin{itemize}
  \item \textbf{UC-CHAT --- Messagerie interne en temps réel :}
    \begin{itemize}
      ...
      \item \textit{Scénario nominal :} Messages de type texte via Socket.IO, persistés en base de données PostgreSQL 16.
     \end{itemize}
...
Le processus opérationnel de modération et d'évaluation manuelle du KYC par l'administrateur, lorsqu'un dossier est mis en attente de revue, suit l'enchaînement d'activités présenté dans la figure~\ref{fig:seq-kyc-review}.
...
\clearpage
\section{Conception de la sécurité biométrique (KYC)}
