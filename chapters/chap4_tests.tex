...
\label{sec:tests-app}
La qualité du système est validée par une suite automatisée \textit{Jest} répartie sur 23 fichiers et complétée par des campagnes manuelles.

\subsection{Tests fonctionnels}
Valident la logique pure métier (authentification, états de réservation).

\subsection{Tests d'intégration}
Valident l'accès à la base de données PostgreSQL et les requêtes HTTP/WebSocket.

\subsection{Tests utilisateurs}
Une session de bêta-test a été conduite auprès de 18 participants recrutés parmi les étudiants de l'Université Abou Bekr Belkaïd de Tlemcen.

\subsection{Tests de performance}
Mesurent les temps de réponse de l'API sous charge.
...
Intégration d'un module d'IA pour optimiser les itinéraires et détecter les comportements inhabituels.

\subsection{Matching intelligent}
Algorithme d'appariement avancé basé sur la notation, les détours acceptés et les wilayas de transit.

\subsection{Paiement CCP / BaridiMob}
Automatisation de la validation des virements BaridiMob par reconnaissance optique des reçus.

\subsection{Système anti-fraude}
Détection de comptes multiples et analyse comportementale des réservations suspectes.

\subsection{Recommandation de trajets}
Moteur de recommandation basé sur l'historique de déplacement de l'utilisateur.
