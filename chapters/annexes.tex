\chapter{Extrait du schéma SQL PostgreSQL}
\label{annexe:sql}
...
\chapter{Extrait de code --- Factory de paiement}
...
\chapter{Extrait du questionnaire utilisateurs}
\label{annexe:questionnaire}

\begin{enumerate}
  \item Quelle est votre fréquence moyenne de déplacement entre wilayas~?
  \item Quel budget mensuel consacrez-vous aux déplacements longue distance~?
  \item Avez-vous déjà utilisé une application de covoiturage~? Si oui, laquelle~?
  \item Quelles sont vos principales préoccupations lors d'un trajet partagé~?
  \item Quelle méthode de paiement privilégiez-vous (CIB, Edahabia, espèces)~?
  \item Seriez-vous prêt à utiliser une application exigeant une vérification biométrique d'identité~?
\end{enumerate}

\chapter{Catalogue complet des captures d'écran}
...
\chapter{Diagrammes de séquence secondaires}
...
\chapter{Diagramme d'activité global}
