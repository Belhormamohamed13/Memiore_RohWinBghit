...
Pour une description ergonomique exhaustive et une analyse détaillée de chacun de ces parcours avec leurs figures et captures d'écran associées, le lecteur est invité à se référer à la section~\ref{sec:screenshots} (« Captures d'écran et interfaces ») de ce chapitre.

\subsection{Guide opératoire et parcours utilisateur (Manuel utilisateur)}
\label{sec:manuel-utilisateur}

Cette sous-section présente le guide opératoire détaillé décrivant l'enchaînement des étapes pour les deux rôles de la plateforme.

\subsubsection{Parcours Passager}
\label{sec:manuel-passager}

\paragraph{Inscription et connexion.}
Lors du premier lancement de l'application, vous devez créer votre compte en saisissant votre numéro de téléphone. Un code de validation à usage unique (OTP) vous est envoyé par SMS. Après sa saisie et sa validation, complétez vos informations de profil (nom, prénom et adresse email). Pour les connexions ultérieures, la saisie du numéro de téléphone et la validation par SMS sont requises pour garantir la sécurité.

\paragraph{Vérification d'identité (KYC).}
...
\paragraph{Recherche et réservation.}
...
\paragraph{Messagerie et suivi temps réel.}
...
\label{sec:manuel-conducteur}

\paragraph{Enregistrement d'un véhicule.}
...
\paragraph{Publication d'un trajet.}
...
\paragraph{Gestion des réservations et portefeuille.}
...
\subsubsection{Schéma général de navigation applicative}
...
\subsection{Schéma SQL de création des tables}
\label{sec:schema-sql-creation}

Pour concrétiser physiquement la base de données relationnelle PostgreSQL, le script SQL suivant présente la création des tables principales de la plateforme, notamment la table des utilisateurs (\texttt{users}) et la table des trajets (\texttt{trips}) avec leurs index et contraintes de clés étrangères associées.
...
L'application mobile expose au total \textbf{62~écrans distincts} couvrant les parcours passager et conducteur. Cette section présente les interfaces les plus représentatives, organisées par parcours fonctionnel (voir les figures~\ref{fig:kyc-screens} à~\ref{fig:settings-legal}).
...
\paragraph{Implémentation du patron Strategy/Factory.}
Afin d'isoler l'orchestration des flux de paiement et de garantir l'extensibilité du module conformément au principe ouvert/fermé (OCP), l'implémentation repose sur une fabrique (\texttt{PaymentFactory}) enregistrant les différentes stratégies disponibles. L'extrait de code~\ref{lst:payment-factory} présente la structure Node.js de cette fabrique.

\begin{lstlisting}[language=Java,caption={Implémentation Node.js du patron \textit{Strategy/Factory} (extrait de \texttt{backend/src/services/patterns/payment/payment.factory.js})},label={lst:payment-factory}]
...
L'ajout d'une nouvelle méthode de paiement (p.~ex.~Baridi~Mob) consiste simplement à créer une classe \texttt{BaridiStrategy} implémentant l'interface commune, puis à l'enregistrer dans la fabrique via \texttt{registerStrategy('baridi', ...)} sans modifier le code de la classe \texttt{PaymentFactory}, garantissant la conformité avec le principe d'ouverture/fermeture.
